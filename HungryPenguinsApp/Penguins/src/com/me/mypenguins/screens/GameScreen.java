...
public class GameScreen implements Screen, InputProcessor{
	...
	public void render(float delta) {
		...
		w.update();
		//System.out.println("velocity: " + w.getPenguin().getLinearVelocity().x);
		if( w.getPenguin().getLinearVelocity().x < 0.002f && w.getPenguin().getPosition().y < WATER_LEVEL){
			paused = true;//System.pause();
			//batch.begin();
			//font = new BitmapFont(Gdx.files.internal("vemana2000.fnt"),
			//Gdx.files.internal("vemana2000.png"), false);
			//font.draw(batch, Integer.toString(w.FISH_COUNT), w.getPenguin().getPosition().x, w.getPenguin().getPosition().y);
			//batch.end();
			//System.out.println(Integer.toString(w.FISH_COUNT));
			/*try {
				Thread.sleep(2000);
			} catch (InterruptedException e) {
	...
	public void drawAllSprites(){
		...
		for( int i = 0; i < w.FISH_SCHOOL; i++ ){
			...
			/*********************************************
			 * Draw Font Sprite for the score board
			 *********************************************/
			if( paused ){
				//System.out.println(Integer.toString(w.FISH_COUNT));
				font = new BitmapFont(Gdx.files.internal("vemana2000.fnt"),
	...
	public boolean touchDown(int x, int y, int pointer, int button) {
		...
		//if( firstRamp ){
		//slopeCheck = w.moveSlope(xpos, ypos);
		//firstRamp = false;//checks to see if the pointer is trying to drag the slope
		//}
		//Gdx.app.log("touchDownDetector", "touchDown at " + xpos + " "+ ypos);
		//Gdx.app.log("slopeCheck", "slope check is  " + slopeCheck);
		if( !slopeCheck ){
			if( firstTouch && touchUp > 1){
				//System.out.println("GestureDetectorTest: tap at " + xpos + ", " + ypos);
				//w.checkPenguinTouch(xpos, ypos);
	...
	public boolean touchUp(int x, int y, int pointer, int button) {
		...
		//if( firstTouch && touchUp > 1){
		w.checkPenguinTouch(xpos, ypos);
		if( /*w.checkPenguinTouch(xpos, ypos)*/ !w.checkSlopeTouch(xpos, ypos) ){

			w.applyForceToPenguin(xpos, ypos);
		}
		firstTouch = false;
		//}
	...
	public boolean touchDragged(int x, int y, int pointer) {
		...
		w.checkPenguinTouch(xpos, ypos);
		if( /*rampDragged*/ rampDragged = w.checkSlopeTouch(xpos, ypos) && !w.penguinClicked ){	//System.out.println("dragged");
			//rampDragged = w.moveSlope(xpos, ypos);//checks to see if the pointer is trying to drag the slope
			//rampDragged = true;
			//System.out.println(rampDragged);
			w.moveSlope(xpos, ypos);
		}

		else if(w.penguinClicked){

			xpos += (renderer.getCam().position.x - (renderer.getCam().viewportWidth / 2));
			ypos += (renderer.getCam().position.y - (renderer.getCam().viewportHeight / 2));

			//System.out.println("dragged");
			w.setPenguinPosition(xpos/*, ypos*/);
		}
		return false;
	}
